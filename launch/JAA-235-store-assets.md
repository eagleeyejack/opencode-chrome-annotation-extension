...
No AI-generated mockups. All five are HTML/CSS artboards rendered in a browser at exact pixels, built from real assets only. Working dir (outside the repo): `/tmp/opencode/promo/`. Brand rule: the OpenCode **wordmark never appears** on our tiles (endorsement risk) - only the word "OpenCode" in copy. Our logo is always `icon.svg` from this repo (dark tile + indigo #818CF8 bubble-crosshair). Type: system mono stack (ui-monospace, SFMono-Regular, Menlo) to echo OpenCode's terminal aesthetic. Overall style: light, minimal, print-editorial - warm off-white #F4F3F1 backgrounds, near-black #1d1d1f text, indigo #818CF8 reserved for the selection-highlight element only. No glow, no neon, no dark gradients. Real captures required before composing: the user's 5 real-UI screenshots (shot list above) - tiles 1 and 2 can compose immediately (logo + copy only), tiles 3-5 need the real captures.

**1. Small promo tile - 440x280**
Artboard: warm off-white #F4F3F1 full-bleed. Left: `icon.svg` mark at 96px (faithful to the repo file: dark tile, indigo selection bubble with crosshair). Right: near-black #1d1d1f bold 24px "Point. Annotate. Ship." Grey mono 12px subline "Send what's broken to your AI agent". Flat, print-like, generous whitespace. QA: text exactly as quoted, PNG 24-bit no alpha.

**2. Marquee promo tile - 1400x560**
Artboard: warm off-white #F4F3F1. Left third (padding 64px): near-black #1d1d1f bold 54px headline "Show your agent what's broken", grey 20px subline "Click an element. The agent gets the screenshot and the selector." Right two-thirds: three real mini-captures of the selection highlight box (indigo outline) cropped from the user's screenshot #2, joined by thin #818CF8 rules on light cards. Hierarchy: one headline, one subline, real evidence. QA: headline never wraps past two lines, captures sharp at 2x source downscale.

**3. Store hero - 1280x720**
Artboard: warm off-white #F4F3F1. Left half: real screenshot #3 (queue with 2-3 items) in a light browser frame with traffic-light dots, scaled to 560px wide, allowed to bleed below the artboard (editorial crop). Right half: `icon.svg` at 64px, near-black #1d1d1f bold 40px "OpenCode Annotate", grey 18px "Point at what's broken. Your agent fixes it." Install line in mono 14px: `{ "plugin": ["opencode-annotate@latest"] }` (verified against package.json - do not invent). QA: install line character-exact, browser frame intentional, no dev chrome visible in the capture.

**4. Social / OG card - 1200x630**
Artboard: warm off-white #F4F3F1. Top safe padding 80px. Near-black #1d1d1f bold 56px two-line headline "Point at what's broken. / Your agent fixes it." Grey 20px line: "OpenCode Annotate - open source, GPL-3.0". Bottom-right: `icon.svg` at 120px. Keep all text inside 80px margins (platform cropping). QA: legible at 300px-wide preview.
